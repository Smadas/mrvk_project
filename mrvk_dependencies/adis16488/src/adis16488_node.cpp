#include <ros/ros.h>
#include <adis16488/mathematics_operations.h>
#include <adis16488/adis_interface.h>
#include <adis16488/moving_average.h>

#include <adis16488/Write.h>
#include <adis16488/Read.h>
#include <std_srvs/Trigger.h>

#include <diagnostic_updater/diagnostic_updater.h>

class Adis16488: public MovingAverage{
public:
	Adis16488() : MovingAverage(10), diagnostic_( ){
        ...
        diagnostic_.add("Adis 16488", this, &Adis16488::diagnostics);
		diagnostic_.setHardwareID("none");

		imuPub = n.advertise<sensor_msgs::Imu>(topicName,100);
		//imuPubTest = n.advertise<sensor_msgs::Imu>("test",100);

		serviceInit = n.advertiseService("init", &Adis16488::initSrv, this);
		serviceWrite = n.advertiseService("write", &Adis16488::writeSrv, this);
		serviceRead = n.advertiseService("read", &Adis16488::readSrv, this);
		serviceCalibrate = n.advertiseService("calibrate", &Adis16488::calibrateSrv, this);
		serviceAutoCalibrate = n.advertiseService("auto_calibrate", &Adis16488::autoCalibrateSrv, this);
		serviceZGyroCalibrate = n.advertiseService("z_gyro_calibrate", &Adis16488::zGyroCalibrateSrv, this);
		serviceRestoringCalibrate = n.advertiseService("restoring_calibrate",&Adis16488::restoringCalibrationSrv, this);
		serviceComputeOffset = n.advertiseService("compute_offset",&Adis16488::computeOffsetSrv, this);
	    ...
	    n.param<double>("status_period", status_period, 1);
    	status_timer = n.createTimer(ros::Duration(status_period), &Adis16488::timerCallback, this); //citanie prebieha periodickym spustanim casovaca
...
private:
	...
	bool initControllRegisters(){
		...
		if (!adis->setSENS_AVG(uint8_t (range), uint8_t(avg))){
			ROS_ERROR("ADIS16488: initial failed in init registers");
			return false;
		}
	...
	bool writeSrv(adis16488::Write::Request  &req, adis16488::Write::Response &res){
	...
	bool readSrv(adis16488::Read::Request  &req, adis16488::Read::Response &res){
