#include <adis16488/adis_interface.h>
...
bool AdisInterface::init(std::string port, int baud){
	...
	if (!active){
		...
		my_serial = new serial::Serial(port, baud, serial::Timeout::simpleTimeout(100));
			...
			ROS_ERROR("ADIS16488: port don't open: %s", e.what());
			active = false;
			return false;
		}
	}
...
bool AdisInterface::setSENS_AVG(uint8_t SENS, uint8_t AVG){
	...
	mutex.unlock();
	if (!success)
		ROS_ERROR("ADIS16488: neuspesny init, zapis do SENS_AVG reg");
...
bool AdisInterface::setSMPL_PRD(uint8_t SMPL){
	...
	mutex.unlock();
	if (!success)
		ROS_ERROR("ADIS16488: neuspesny init, zapis do SMPL_PRD reg");
...
bool AdisInterface::setMSC_CTRL(uint8_t MSC){
	...
	mutex.unlock();
	if (!success)
		ROS_ERROR("ADIS16488: neuspesny init, zapis do MSC_CTRL reg");
...
bool AdisInterface::restoringCalibration(){
	...
	if (!success)
		ROS_ERROR("ADIS16488: neuspesne mazanie offset registrov");
	return success;
}
...
bool AdisInterface::autoCalibrate(){
	...
	mutex.lock();
	ROS_WARN("ADIS16488: prebieha kratka kalibracia nehybat 2 sekundy zo zoriadenim");
	...
	if (!success)
		ROS_ERROR("ADIS16488: neuspesne auto kalibracia");
	else ROS_INFO("ADIS16488: kratka kalibracia uspesne dokoncena");
	return success;
}
...
bool AdisInterface::calibrate(){
	...
	calibrationRunning.lock();
	mutex.lock();
	ROS_WARN("ADIS16488: prebieha kalibracia nehybat 30 sekund zo zoriadenim");
	sleep(2);
	...
	if (!success)
		ROS_ERROR("ADIS16488: neuspesna kalibracia");
	else ROS_INFO("ADIS16488: kalibracia uspesne dokoncena");
	return success;
}
...
bool AdisInterface::calibrationGyroZ(){
	...
	if (!writeRegister(ZGYRO_OFF, (uint8_t)(offset >> 8),(uint8_t)(offset & 0x00FF))){
		success = false;
		ROS_ERROR("ADIS16488: neuspesny zapis do registru ZGYRO_OFF");
	} else
		ROS_INFO("ADIS16488: kratka kalibracia uspesne dokoncena");
		...
		try{
			...
			int count  = my_serial->write(&write_data,1);
				...
				if (count != 1){
					...
					mutex.unlock();
					ROS_ERROR("ADIS16488: read temperature: zly pocet zapisanych bytov %d", count);
					return temperature;
				}
				...
				if (count != 6){
					...
					mutex.unlock();
					ROS_ERROR("ADIS16488: read temperature: zly pocet precitanych bytov %d", count);
					return temperature;
				}

		}catch (std::exception& e){
			mutex.unlock();
			ROS_ERROR("ADIS16488: exception: %s",e.what());
			active = false;
			return temperature;
			}
	}
	...
	static uint8_t read_data[12];
	 ...
	 try{
		...
		if (count != 1){
			...
			//ROS_ERROR("ADIS16488: read axes: zly pocet zapisanych bytov %d", count);
			return false;
		}
		...
		if (count != 12){
			...
			//ROS_ERROR("ADIS16488: read axes: zly pocet precitanych bytov %d", count);
			return false;
		}
...
bool AdisInterface::writeRegister(uint8_t reg, uint8_t hi, uint8_t lo){
	...
	try{
		...
		if (!success)
			ROS_ERROR("ADIS16488: neuspesny init, zapis do registra %X", reg);
...
bool AdisInterface::readRegister(uint8_t reg, uint8_t *read_data){
	...
	try{
		...
		if (count != 2){
				ROS_ERROR("ADIS16488: read_register: zly pocet zapisanych bytov %d", count);
				return false;
		}
		count = my_serial->read(read_data,2);

		if (count != 2){
			ROS_ERROR("ADIS16488: read_register: zly pocet precitanych bytov %d", count);
			return false;
		}
		else
